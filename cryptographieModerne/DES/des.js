...
function encrypt() {
    ...
    plainText = BitArray.fromAscii(plainText);//DES.generateRandomKey();
    key = BitArray.fromAscii(key);//DES.generateRandomMessage();


    // Simple "encryption" by shifting characters (just for demo purposes)
    let encrypted = binaryToText(DES.encrypt( key,plainText).final.toString());
...
function decrypt() {
    ...
    encryptedText = BitArray.fromAscii(encryptedText);//DES.generateRandomKey();
    key = BitArray.fromAscii(key);//DES.generateRandomMessage();
    
    
    // Simple "decryption" (just for demo purposes)
    let decrypted = binaryToText(DES.decrypt( key,encryptedText).final.toString());;
...
function binaryToText(binaryStr) {
    return binaryStr.match(/.{8}/g) // Split into 8-bit chunks
        .map(byte => String.fromCharCode(parseInt(byte, 2))) // Convert to character
        .join('');
}


function textTo64BitArray(text) {
    let encoder = new TextEncoder();
    let bytes = encoder.encode(text); // UTF-8 bytes

    // Ensure exactly 8 bytes (64 bits)
    let paddedBytes = new Uint8Array(8);
    paddedBytes.set(bytes.slice(0, 8)); // Truncate or pad with zeros
